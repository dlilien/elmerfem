...
!>          stress from an anisotropic flow solution    
!> 2D SDOFs = 4 (S11, S22, S33, S12)                               
!> 3D SDOFs = 6 (S11, S22, S33, S12, S23, S31)                     
!> Keywords : Cauchy (Logical),                                    
!------------------------------------------------------------------------------
RECURSIVE SUBROUTINE ComputeDevStressAI( Model,Solver,dt,TransientSimulation )
!------------------------------------------------------------------------------
  ...
  SAVE LocalVelo, LocalP, dim
      ...
      IF (.NOT.GotIt) THEN
        WRITE(Message,'(A)') 'VariableGas Constant  not found. &
                     &Setting to 8.314'
        CALL INFO('StressAI', Message, level=20)
        Wn(7) = 8.314
      ELSE
...
!!!! Restricted to the Power Law case
           ...
           CALL LocalAIMatrix(COMP, LocalMassMatrix, LocalStiffMatrix, LocalForce, &
                K1, K2, E1, E2, E3, Wn, MinSRInvariant, LocalVelo, LocalP, &
                LocalFluidity, LocalTemperature, CurrentElement, n, &
                ElementNodes, Cauchy, Isotropic)
...
!------------------------------------------------------------------------------
  SUBROUTINE LocalAIMatrix(COMP, MassMatrix, StiffMatrix, ForceVector, &
       NodalK1, NodalK2, NodalE1, NodalE2, NodalE3, Wn, MinSRInvariant, &
       NodalVelo, NodalP, NodalFluidity, &
       NodalTemp, Element, n, Nodes, Cauchy, Isotropic )
!------------------------------------------------------------------------------
...
! non relative viscosities
    ...
    END IF
     ...
     Stress = StressTensor(INDx(COMP), INDy(COMP)) 
...
!------------------------------------------------------------------------------
END SUBROUTINE ComputeDevStressAI
!------------------------------------------------------------------------------
